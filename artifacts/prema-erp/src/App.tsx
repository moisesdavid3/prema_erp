...
function Metric({ label, value, detail, icon: Icon, tone = 'cream' }: { label: string; value: string; detail?: string; icon: typeof Box; tone?: 'cream' | 'green' | 'orange' }) {
  ...
  return <div className={`rounded-2xl border p-4 ${tones[tone]} rise-in`} data-testid={`metric-${label.toLowerCase().replaceAll(' ', '-')}`}><div className="flex items-start justify-between"><p className="text-sm font-semibold text-[hsl(var(--muted-foreground))]">{label}</p><span className="grid h-9 w-9 place-items-center rounded-xl bg-[hsl(var(--background)/.6)] text-[hsl(var(--primary))]"><Icon size={18} /></span></div><p className="mt-2 break-all font-mono-app text-2xl font-bold leading-tight tracking-tight">{value}</p>{detail && <p className="mt-1 text-xs text-[hsl(var(--muted-foreground))]">{detail}</p>}</div>;
}
  ...
  return <Shell><PageHeading eyebrow="Resumen del negocio" title={activeCompany?.name ?? 'Tu negocio'} description="Esto es lo que está pasando con tu negocio hoy." action={<Link href="/app/venta" className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-[hsl(var(--primary))] px-5 text-sm font-bold text-[hsl(var(--primary-foreground))] shadow-[0_4px_0_hsl(var(--primary-border))]" data-testid="link-dashboard-sale"><ShoppingCart size={18} /> Registrar venta</Link>} />
    {dashboard.isLoading ? <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{[1, 2, 3, 4].map((n) => <div key={n} className="h-36 animate-pulse rounded-2xl bg-[hsl(var(--muted))]" />)}</div> :
      dashboard.isError ? <StatusMessage text="No pudimos cargar tu resumen. Revisa tu conexión e inténtalo de nuevo." onRetry={() => dashboard.refetch()} /> :
        <><div className={`grid gap-4 sm:grid-cols-2 ${isPrema ? 'xl:grid-cols-5' : 'xl:grid-cols-6'}`}><Metric label="Productos" value={String(data?.productCount ?? 0)} detail={`${data?.totalUnits ?? 0} unidades en existencia`} icon={Box} /><Metric label="Ventas de hoy" value={money(data?.todaySalesTotal ?? 0)} detail={`${data?.todaySalesCount ?? 0} ventas registradas`} icon={CircleDollarSign} tone="green" /><Metric label="Valor para vender" value={money(data?.inventorySaleValue ?? 0)} detail="Si vendes todo tu inventario" icon={TrendingUp} tone="orange" /><Metric label="Costo del inventario" value={money(data?.inventoryCostValue ?? 0)} detail="Invertido en todo tu stock" icon={Wallet} tone="orange" /><Metric label="Ganancia posible" value={money(data?.potentialProfit ?? 0)} detail="Antes de gastos adicionales" icon={Sparkles} />{!isPrema && <div className="rounded-2xl border bg-[hsl(var(--secondary)/.7)] p-4 rise-in" data-testid="metric-deuda-moises-david"><div className="flex items-start justify-between"><p className="text-sm font-semibold text-[hsl(var(--muted-foreground))]">Deuda Moises David</p><span className="grid h-9 w-9 place-items-center rounded-xl bg-[hsl(var(--background)/.6)] text-[hsl(var(--primary))]"><Wallet size={18} /></span></div><p className="mt-2 break-all font-mono-app text-2xl font-bold leading-tight tracking-tight">{money(data?.deudaMoisesDavid ?? 0)}</p>{data?.canEditDeudaMoises ? <button onClick={() => setDebtModal(true)} className="mt-2 text-xs font-semibold text-[hsl(var(--primary))] underline underline-offset-2" data-testid="button-edit-debt">Actualizar deuda</button> : <p className="mt-2 text-xs text-[hsl(var(--muted-foreground))]">Valor pendiente</p>}</div>}</div>
...
function ProductModal({ product, onClose }: { product?: Product; onClose: () => void }) {
  ...
  const [form, setForm] = useState({ name: product?.name || '', supplier: product?.supplier || '', category: product?.category || '', content: product?.content || '', cost: String(product?.cost ?? ''), salePrice: String(product?.salePrice ?? ''), stock: product ? String(product.stock) : '', minimumStock: String(product?.minimumStock ?? 2), active: product?.active ?? true, stockDate: toLocalDateString(new Date()) });
  const [error, setError] = useState('');
  ...
  const submit = (e: React.FormEvent) => { e.preventDefault(); if (!form.name.trim() || Number(form.salePrice) < 0 || Number(form.stock) < 0) { setError('Revisa los datos. El nombre y los precios son necesarios.'); return; } setError(''); const done = () => { qc.invalidateQueries({ queryKey: getListProductsQueryKey() }); qc.invalidateQueries({ queryKey: getGetDashboardQueryKey() }); onClose(); }; const supplier = form.supplier.trim() || undefined; const category = form.category.trim() || undefined; const content = form.content.trim() || undefined; if (isEdit) update.mutate({ id: product.id, data: { name: form.name.trim(), supplier, category, content, cost: Number(form.cost), salePrice: Number(form.salePrice), stock: Number(form.stock), minimumStock: Number(form.minimumStock), active: form.active, stockDate: Number(form.stock) !== product.stock ? dateInputToISO(form.stockDate) : undefined } }, { onSuccess: done, onError: () => setError('No se pudo guardar el producto.') }); else create.mutate({ data: { name: form.name.trim(), supplier, category, content, cost: Number(form.cost), salePrice: Number(form.salePrice), initialStock: Number(form.stock), minimumStock: Number(form.minimumStock), active: form.active, initialStockDate: Number(form.stock) > 0 ? dateInputToISO(form.stockDate) : undefined } }, { onSuccess: done, onError: () => setError('No se pudo crear el producto.') }); };
        ...
        return <div key={p.id} className={`border-b p-5 last:border-0 md:grid md:items-center md:gap-4 md:px-5 md:py-4 ${isPrema ? 'md:grid-cols-[1fr_110px_110px_150px]' : 'md:grid-cols-[1fr_110px_110px_130px_150px]'}`} data-testid={`row-product-${p.id}`}>
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0 flex-1">
              <p className="whitespace-normal break-words font-bold md:truncate">{[p.name, p.content].filter(Boolean).join(' x ')}{p.active === false && <span className="ml-2 inline-block rounded-full bg-[hsl(var(--muted))] px-2 py-0.5 text-[10px] font-bold text-[hsl(var(--muted-foreground))]">Inactivo</span>}</p>
...
function SalePage() {
  ...
  const matches = useMemo(() => (products.data || []).filter((p) => p.active !== false && (allowNegative || p.stock > 0) && (p.name.toLowerCase().includes(query.trim().toLowerCase()) || (p.supplier || '').toLowerCase().includes(query.trim().toLowerCase()))), [products.data, query, allowNegative]);
...
function PurchaseModal({ onClose }: { onClose: () => void }) {
  ...
  const matches = useMemo(() => (products.data || []).filter((p) => p.active !== false && (p.name.toLowerCase().includes(query.trim().toLowerCase()) || (p.supplier || '').toLowerCase().includes(query.trim().toLowerCase()))), [products.data, query]);
...
function StockoutModal({ onClose }: { onClose: () => void }) {
  ...
  return <div className="fixed inset-0 z-50 flex items-end justify-center bg-[hsl(var(--foreground)/.45)] p-0 sm:grid sm:place-items-center sm:p-4" role="dialog" aria-modal="true"><form onSubmit={submit} className="max-h-[92dvh] sm:max-h-[calc(100dvh-2rem)] w-full max-w-2xl flex-col overflow-auto rounded-t-2xl sm:rounded-2xl border bg-[hsl(var(--card))] p-4 sm:p-6 shadow-2xl"><div className="flex items-start justify-between"><div><p className="font-mono-app text-[10px] uppercase tracking-[.16em] text-[hsl(var(--primary))]">Ajuste de inventario</p><h2 className="mt-1 font-display text-2xl sm:text-3xl font-bold">Dar de baja</h2><p className="mt-1 text-sm text-[hsl(var(--muted-foreground))]">Registra productos vencidos o dañados; se restarán del stock.</p></div><button type="button" onClick={onClose} className="rounded-lg p-2 text-[hsl(var(--muted-foreground))] hover:bg-[hsl(var(--muted))]" data-testid="button-close-stockout-modal"><X size={20} /></button></div>
    ...
    {items.length > 0 && <div className="mt-4 divide-y rounded-xl bg-[hsl(var(--muted)/.4)] px-3">{items.map((item, idx) => { const p = products.data?.find((x) => x.id === item.productId); return <div key={item.productId} className="flex items-center gap-3 py-2.5 text-sm" data-testid={`stockout-item-${item.productId}`}><span className="min-w-0 flex-1 truncate font-semibold">{p?.name || `#${item.productId}`}{p?.content ? ` · ${p.content}` : ''}</span>{item.reason && <span className="rounded-full bg-orange-100 px-2 py-0.5 text-[10px] font-bold text-orange-700">{item.reason}</span>}<span className="font-mono-app text-xs text-[hsl(var(--muted-foreground))]">× {item.quantity}</span><span className="font-bold">{money(item.quantity * item.unitCost)}</span><button type="button" onClick={() => setItems(items.filter((_, i) => i !== idx))} className="rounded-lg p-1 text-[hsl(var(--muted-foreground))] hover:bg-[hsl(var(--destructive)/.1)] hover:text-[hsl(var(--destructive))]" title="Quitar" data-testid={`button-remove-stockout-${item.productId}`}><Trash2 size={15} /></button></div>; })}</div>}
    <div className="mt-4 flex items-center justify-between"><div className="text-sm text-[hsl(var(--muted-foreground))]">Total a dar de baja</div><strong className="text-lg">{money(total)}</strong></div>
...
function Purchases() {
  const [supplierFilter, setSupplierFilter] = useState(''); const [monthFilter, setMonthFilter] = useState('');
  const purchases = useListPurchases({ period: 'all' }); const qc = useQueryClient(); const del = useDeletePurchase();
  const [modal, setModal] = useState(false); const [importModal, setImportModal] = useState(false); const [expanded, setExpanded] = useState<number | null>(null); const [confirm, setConfirm] = useState<Purchase | null>(null); const [page, setPage] = useState(1);
  const PER_PAGE = 10;
  const filtered = useMemo(() => (purchases.data || []).filter((p) => (!supplierFilter || p.supplier === supplierFilter) && (!monthFilter || p.date.slice(0, 7) === monthFilter)), [purchases.data, supplierFilter, monthFilter]);
  const pageCount = Math.max(1, Math.ceil(filtered.length / PER_PAGE));
  const currentPage = Math.min(page, pageCount);
  const paged = filtered.slice((currentPage - 1) * PER_PAGE, currentPage * PER_PAGE);
  const supplierOptions = useMemo(() => Array.from(new Set((purchases.data || []).map((p) => p.supplier).filter((s): s is string => !!s))).sort((a, b) => a.localeCompare(b)), [purchases.data]);
  const monthOptions = useMemo(() => { const opts: { value: string; label: string }[] = []; const now = new Date(); for (let i = 0; i < 12; i++) { const d = new Date(now.getFullYear(), now.getMonth() - i, 1); const v = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}`; const l = d.toLocaleDateString('es-CO', { year: 'numeric', month: 'long' }); opts.push({ value: v, label: l.charAt(0).toUpperCase() + l.slice(1) }); } return opts; }, []);
  const totalSpent = filtered.reduce((sum, p) => sum + p.total, 0);
  const totalUnits = filtered.reduce((sum, p) => sum + p.totalItems, 0);
  return <Shell><PageHeading eyebrow="Surtir inventario" title="Compras" description="Registra las entradas de mercancía y el stock se actualiza solo." action={<div className="flex items-center justify-end gap-2"><Link href="/app/proveedores" className="inline-flex min-h-[40px] items-center justify-center gap-2 rounded-xl border bg-[hsl(var(--card))] px-4 text-sm font-bold text-[hsl(var(--foreground))] shadow-sm" data-testid="button-new-supplier-purchases"><PackagePlus size={16} /> Proveedores</Link><Button onClick={() => setModal(true)} data-testid="button-new-purchase"><Plus size={16} /> Nueva compra</Button></div>} />
    <div className="mb-6 flex flex-col gap-3 rounded-2xl border bg-[hsl(var(--card))] p-3 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-3 text-sm"><span className="font-semibold text-[hsl(var(--muted-foreground))]">Invertido:</span><strong className="text-lg">{money(totalSpent)}</strong><span className="hidden h-5 w-px bg-[hsl(var(--muted))] sm:block" /><span className="text-[hsl(var(--muted-foreground))]">{totalUnits} unidades entradas</span></div><div className="flex flex-col gap-2 sm:flex-row sm:items-center"><label className="relative"><select value={supplierFilter} onChange={(e) => { setSupplierFilter(e.target.value); setPage(1); }} className="h-10 w-full appearance-none rounded-lg border bg-[hsl(var(--background))] px-3 pr-8 text-sm font-bold outline-none sm:w-44" data-testid="select-purchase-supplier"><option value="">Todos los proveedores</option>{supplierOptions.map((s) => <option key={s} value={s}>{s}</option>)}</select><ChevronDown size={15} className="pointer-events-none absolute right-2.5 top-3" /></label><label className="relative"><select value={monthFilter} onChange={(e) => { setMonthFilter(e.target.value); setPage(1); }} className="h-10 w-full appearance-none rounded-lg border bg-[hsl(var(--background))] px-3 pr-8 text-sm font-bold outline-none sm:w-44" data-testid="select-purchase-month"><option value="">Todos los meses</option>{monthOptions.map((m) => <option key={m.value} value={m.value}>{m.label}</option>)}</select><ChevronDown size={15} className="pointer-events-none absolute right-2.5 top-3" /></label></div></div>
    {purchases.isLoading ? <div className="grid gap-3">{[1, 2, 3].map((n) => <div key={n} className="h-28 animate-pulse rounded-2xl bg-[hsl(var(--muted))]" />)}</div> : purchases.isError ? <StatusMessage text="No pudimos cargar las compras." onRetry={() => purchases.refetch()} /> : !purchases.data?.length ? <StatusMessage type="empty" text="Todavía no hay compras registradas. Crea la primera para surtir tu inventario." /> : !filtered.length ? <StatusMessage type="empty" text="No hay compras con esos filtros." /> : <><div className="grid gap-3">{paged.map((purchase) => <div key={purchase.id} className="overflow-hidden rounded-2xl border bg-[hsl(var(--card))]" data-testid={`purchase-${purchase.id}`}><div className="flex flex-wrap items-center gap-3 px-5 py-4"><button type="button" onClick={() => setExpanded(expanded === purchase.id ? null : purchase.id)} className="flex min-w-0 flex-1 items-center gap-3 text-left"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-[hsl(var(--secondary))] text-[hsl(var(--primary))]"><ShoppingBasket size={18} /></span><span className="min-w-0 flex-1"><span className="block truncate font-bold">{purchase.supplier || 'Compra sin proveedor'}</span><span className="block text-xs text-[hsl(var(--muted-foreground))]">{dateLabel(purchase.date)}{purchase.invoiceNumber ? ` · Factura ${purchase.invoiceNumber}` : ''} · {purchase.totalItems} unidades</span></span><ChevronDown size={16} className={`shrink-0 text-[hsl(var(--muted-foreground))] transition-transform ${expanded === purchase.id ? 'rotate-180' : ''}`} /></button><span className="text-lg font-bold">{money(purchase.total)}</span><button type="button" onClick={() => setConfirm(purchase)} className="rounded-lg p-2 text-[hsl(var(--muted-foreground))] hover:bg-[hsl(var(--destructive)/.1)] hover:text-[hsl(var(--destructive))]" title="Anular compra" data-testid={`button-delete-purchase-${purchase.id}`}><Trash2 size={16} /></button></div>{expanded === purchase.id && <div className="border-t bg-[hsl(var(--muted)/.3)] px-5 py-3">{purchase.items.map((item) => <div key={item.productId} className="flex items-center justify-between gap-3 py-1.5 text-sm"><span className="min-w-0 flex-1 truncate text-[hsl(var(--muted-foreground))]">{item.quantity} × {item.productCode || ''} {item.productName}{item.content ? ` x ${item.content}` : ''}</span><span className="font-semibold">{money(item.subtotal)}</span></div>)}</div>}</div>)}</div>  {paged.length > 0 && pageCount > 1 && <div className="mt-4 flex items-center justify-center gap-3"><button type="button" onClick={() => setPage(currentPage - 1)} disabled={currentPage === 1} className="h-9 rounded-lg border bg-[hsl(var(--card))] px-3 text-sm font-bold outline-none disabled:cursor-not-allowed disabled:opacity-40" data-testid="button-purchases-prev">← Anterior</button><span className="text-sm font-semibold text-[hsl(var(--muted-foreground))]" data-testid="purchases-page-label">Página {currentPage} de {pageCount}</span><button type="button" onClick={() => setPage(currentPage + 1)} disabled={currentPage === pageCount} className="h-9 rounded-lg border bg-[hsl(var(--card))] px-3 text-sm font-bold outline-none disabled:cursor-not-allowed disabled:opacity-40" data-testid="button-purchases-next">Siguiente →</button></div>}</>}
...
function SalesReport() {
  const [dateFrom, setDateFrom] = useState(() => toLocalDateString(new Date())); const [dateTo, setDateTo] = useState(() => toLocalDateString(new Date())); const [month, setMonth] = useState(''); const monthOptions = useMemo(() => { const opts: { value: string; label: string }[] = []; const now = new Date(); for (let i = 0; i < 12; i++) { const d = new Date(now.getFullYear(), now.getMonth() - i, 1); const v = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}`; const l = d.toLocaleDateString('es-CO', { year: 'numeric', month: 'long' }); opts.push({ value: v, label: l.charAt(0).toUpperCase() + l.slice(1) }); } return opts; }, []);
  const salesParams = dateFrom && dateTo ? { from: new Date(dateFrom + 'T00:00:00-05:00').toISOString(), to: new Date(dateTo + 'T23:59:59.999-05:00').toISOString() } : month ? { from: new Date(month + '-01T00:00:00-05:00').toISOString(), to: new Date(new Date(month + '-01T00:00:00-05:00').setMonth(new Date(month + '-01T00:00:00-05:00').getMonth() + 1) - 1).toISOString() } : { period: 'all' as const }; const sales = useGetSalesReport(salesParams); const qc = useQueryClient(); const deleteSale = useDeleteSale(); const { activeCompany } = useCompany(); const isPrema = activeCompany?.id === 2; const [confirmSale, setConfirmSale] = useState<number | null>(null);
  ...
  const incomeTotal = useMemo(() => (sales.data?.sales ?? []).filter((s) => s.paymentMethod !== 'Crédito').reduce((sum, s) => sum + productTotal(s), 0) + (sales.data?.payments ?? []).reduce((sum, p) => sum + p.amount, 0), [sales.data]);
  const creditSalesTotal = useMemo(() => (sales.data?.sales ?? []).filter((s) => s.paymentMethod === 'Crédito').reduce((sum, s) => sum + productTotal(s), 0), [sales.data]);
  const exportRows = () => {
          ...
          <div className="space-y-1.5 rounded-xl bg-[hsl(var(--muted)/.4)] p-3">
            {sale.items.map((item) => (
              <div key={item.productId} className="flex items-center justify-between gap-3 text-sm">
                ...
                <span className="shrink-0 text-[hsl(var(--muted-foreground))]">{money(item.subtotal)}</span>
              </div>
            ))}
          </div>
        </div>
              ...
              <tbody>
                {clientGroups.map((g, i) => (
                  <tr key={i} className="border-b last:border-0 hover:bg-[hsl(var(--muted)/.3)]" data-testid={`cartera-client-${i}`}>
                    ...
                    <td className="px-4 py-3 text-right text-green-600">{money(g.paid)}</td>
                    <td className="px-4 py-3 text-right font-bold">{money(g.remaining)}</td>
              ...
              {clientGroups.map((g, i) => (
                <div key={i} className="p-4 space-y-2.5" data-testid={`cartera-client-${i}`}>
                  ...
                  <div className="grid grid-cols-2 gap-2 text-center">
                    <div className="rounded-lg bg-green-50 py-1.5"><p className="text-[10px] text-[hsl(var(--muted-foreground))]">Pagado</p><p className="text-xs font-bold text-green-600">{money(g.paid)}</p></div>
                    <div className="rounded-lg bg-orange-50 py-1.5"><p className="text-[10px] text-[hsl(var(--muted-foreground))]">Saldo</p><p className="text-xs font-bold text-orange-600">{money(g.remaining)}</p></div>
                  </div>
      ...
      <div className="w-full max-w-5xl rounded-t-2xl sm:rounded-2xl border bg-[hsl(var(--card))] shadow-2xl max-h-[92dvh] flex flex-col">
        ...
        <div className="px-5 sm:px-6 pt-4">
          <div className="grid grid-cols-3 gap-2 sm:gap-3">
            <div className="rounded-xl bg-[hsl(var(--muted)/.4)] p-2.5 sm:p-3 text-center"><p className="text-[10px] sm:text-xs text-[hsl(var(--muted-foreground))]">Total</p><p className="mt-0.5 text-base sm:text-lg font-bold">{money(total)}</p></div>
            <div className="rounded-xl bg-[hsl(var(--accent)/.5)] p-2.5 sm:p-3 text-center"><p className="text-[10px] sm:text-xs text-[hsl(var(--muted-foreground))]">Pagado</p><p className="mt-0.5 text-base sm:text-lg font-bold text-green-600">{money(paid)}</p></div>
            <div className="rounded-xl bg-[hsl(var(--secondary)/.6)] p-2.5 sm:p-3 text-center"><p className="text-[10px] sm:text-xs text-[hsl(var(--muted-foreground))]">Pendiente</p><p className="mt-0.5 text-base sm:text-lg font-bold text-orange-600">{money(remaining)}</p></div>
          </div>
            ...
            <div className="mt-2.5 flex flex-wrap gap-2">
              {companyBreakdown.map(([cid, cb]) => (
                <div key={cid} className="flex-1 min-w-[120px] rounded-xl border bg-[hsl(var(--muted)/.2)] px-3 py-2">
                  ...
                  <p className="mt-1 text-xs sm:text-sm font-bold text-orange-600">Pendiente: {money(cb.remaining)}</p>
                  <p className="text-[10px] sm:text-xs text-green-600">Pagado: {money(cb.paid)}</p>
                </div>
              ))}
            </div>
          )}
                ...
                {movements.map((m) => {
                  ...
                  if (m.kind === 'manual' && m.mc) {
                    ...
                    return <tr key={m.key} className="border-b last:border-0">
                      ...
                      <td className="px-3 py-2 text-right font-bold">{money(mc.total)}</td>
                      <td className="px-3 py-2 text-right text-green-600">{money(mc.paid)}</td>
                      <td className="px-3 py-2 text-right">{money(mc.total - mc.paid)}</td>
                  ...
                  if (m.kind === 'payment' && m.payment) {
                    ...
                    return <tr key={m.key} className="border-b last:border-0 bg-green-50/50">
                      ...
                      <td className="px-3 py-2 text-right font-bold text-green-600">-{money(p.amount)}</td>
                      <td className="px-3 py-2 text-right text-green-600">{money(p.amount)}</td>
                      <td className="px-3 py-2"></td>
...
<span className="font-bold">{s.isDelivery ? <span className="text-right"><span className="block">{money(s.total - (s.deliveryCost || 0))}</span><span className="block text-[10px] font-semibold text-[hsl(var(--muted-foreground))]">Domicilio {money(s.deliveryCost || 0)}</span></span> : money(s.total)}</span>
              ...
              if (m.kind === 'manual' && m.mc) {
                ...
                return <div key={m.key} className="rounded-xl border bg-purple-50/50 p-3">
                  ...
                  <div className="mt-2 flex items-center justify-between text-xs">
                    <span className="font-bold">{money(mc.total)}</span>
              ...
              if (m.kind === 'payment' && m.payment) {
                ...
                return <div key={m.key} className="rounded-xl border border-green-200 bg-green-50/50 p-3">
                  ...
                  <div className="mt-2 flex items-center justify-between gap-2">
                    <p className="text-xs font-bold text-green-600">+{money(p.amount)}</p>
