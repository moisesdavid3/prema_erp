...
function Purchases() {
  ...
  return <Shell><PageHeading eyebrow="Surtir inventario" title="Compras" description="Registra las entradas de mercancía y el stock se actualiza solo." action={<div className="flex items-center justify-end gap-2"><Link href="/app/proveedores" className="inline-flex min-h-[40px] items-center justify-center gap-2 rounded-xl border bg-[hsl(var(--card))] px-4 text-sm font-bold text-[hsl(var(--foreground))] shadow-sm" data-testid="button-new-supplier-purchases"><PackagePlus size={16} /> Proveedores</Link><Button onClick={() => setModal(true)} data-testid="button-new-purchase"><Plus size={16} /> Nueva compra</Button></div>} />
    <div className="mb-6 flex flex-col gap-3 rounded-2xl border bg-[hsl(var(--card))] p-3 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-3 text-sm"><span className="font-semibold text-[hsl(var(--muted-foreground))]">Invertido:</span><strong className="text-lg">{money(totalSpent)}</strong><span className="hidden h-5 w-px bg-[hsl(var(--muted))] sm:block" /><span className="text-[hsl(var(--muted-foreground))]">{totalUnits} unidades entradas</span></div><label className="relative"><select value={period} onChange={(e) => setPeriod(e.target.value as typeof period)} className="h-10 w-full appearance-none rounded-lg border bg-[hsl(var(--background))] px-3 pr-8 text-sm font-bold outline-none sm:w-48" data-testid="select-purchase-period"><option value="today">Hoy</option><option value="last7">Últimos 7 días</option><option value="thisMonth">Este mes</option><option value="previousMonth">Mes pasado</option><option value="all">Todo el tiempo</option></select><ChevronDown size={15} className="pointer-events-none absolute right-2.5 top-3" /></label></div>
...
function SalesReport() {
  const [dateFrom, setDateFrom] = useState(() => toLocalDateString(new Date())); const [dateTo, setDateTo] = useState(() => toLocalDateString(new Date())); const [month, setMonth] = useState('__today__'); const monthOptions = useMemo(() => { const opts: { value: string; label: string }[] = []; const now = new Date(); for (let i = 0; i < 12; i++) { const d = new Date(now.getFullYear(), now.getMonth() - i, 1); const v = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}`; const l = d.toLocaleDateString('es-CO', { year: 'numeric', month: 'long' }); opts.push({ value: v, label: l.charAt(0).toUpperCase() + l.slice(1) }); } return opts; }, []); const todayFrom = () => new Date(`${toLocalDateString(new Date())}T00:00:00-05:00`).toISOString(); const todayTo = () => new Date(`${toLocalDateString(new Date())}T23:59:59.999-05:00`).toISOString();
  const salesParams = dateFrom && dateTo ? { from: new Date(dateFrom + 'T00:00:00-05:00').toISOString(), to: new Date(dateTo + 'T23:59:59.999-05:00').toISOString() } : month === '__today__' ? { from: todayFrom(), to: todayTo() } : month ? { from: new Date(month + '-01T00:00:00-05:00').toISOString(), to: new Date(new Date(month + '-01T00:00:00-05:00').setMonth(new Date(month + '-01T00:00:00-05:00').getMonth() + 1) - 1).toISOString() } : { period: 'all' as const }; const sales = useGetSalesReport(salesParams); const qc = useQueryClient(); const deleteSale = useDeleteSale(); const { activeCompany } = useCompany(); const isPrema = activeCompany?.id === 2; const [confirmSale, setConfirmSale] = useState<number | null>(null);
